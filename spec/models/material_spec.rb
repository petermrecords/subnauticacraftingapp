...
RSpec.describe Material, type: :model do
  ...
  context "spaces must be positive numbers or null" do
    it "is valid with a positive number of inventory spaces" do
      @material.inventory_spaces = rand(9999) + 1
      expect(@material.valid?).to eq(true)
    end
    ...
    it "is invalid with a negative number of growbed spaces" do
      @material.growbed_spaces = -(rand(9999) + 1)
      @material.valid?
      expect(@material.errors[:growbed_spaces]).not_to eq([])
    end

    it "is valid with a positive number of storage spaces provided" do
      @material.storage_spaces_provided = rand(9999) + 1
      expect(@material.valid?).to eq(true)
    end
  ...
  context "provides practical information about its characteristics ingame" do
    it "is not craftable if it does not require materials" do
      expect(@material.craftable?).to eq(false)
    end

    it "is not carryable if it does not take up inventory space" do
  ...
  context "interacts with a blueprint" do
    before(:each) do
      @material2 = Material.create({
    ...
    it "is valid if it can be crafted and produces a positive quantity" do
      @material.number_produced = rand(9999) + 1
      @material.valid?
  ...
  context "interacts with a byproduct" do
    before(:each) do
      @material2 = Material.create({
        material_name: "Test Material II",
        material_type: "Test Type"
